...
this.nodeId:b.p=this.downloadId;var d=L.decrypt(this.key);(this.api||ob).request(b,function(a,b){function c(){var a=Math.min(b.s-1,h+m);if(!(a<=h)){var d=D(b.g+"/"+h+"-"+a);d.on("end",c);r.append(d,{contentLength:a-h});h=a;m=Math.max(m+131072,1048576);4>++e&&setTimeout(c,1E3)}}if(a)return d.emit("error",a);var e=0,h=0,m=131072,r=Nb.create();c();r.pipe(d);var p=0;d.on("data",function(a){p+=a.length;d.emit("progress",{bytesLoaded:p,bytesTotal:b.s})})});a&&fb(d,a);return d}},{key:"delete",value:function(a){if(!this.nodeId)return I.nextTick(function(){a(Error("delete is only supported on files with node ID-s"))});
