...
this.nodeId:b.p=this.downloadId;var d=L.decrypt(this.key);(this.api||ob).request(b,function(a,b){function c(){var a=Math.min(b.s,h+m);if(!(a<=h)){var d=D(b.g+"/"+h+"-"+(a-1));d.on("end",c);r.append(d,{contentLength:a-h});h=a;m=Math.max(m+131072,1048576);4>++e&&setTimeout(c,1E3)}}if(a)return d.emit("error",a);var e=0,h=0,m=131072,r=Nb.create();c();r.pipe(d);var p=0;d.on("data",function(a){p+=a.length;d.emit("progress",{bytesLoaded:p,bytesTotal:b.s})})});a&&fb(d,a);return d}},{key:"delete",value:function(a){if(!this.nodeId)return I.nextTick(function(){a(Error("delete is only supported on files with node ID-s"))});
...
!0),a);return new Ta(c)};O.packAttributes=function(e){e=JSON.stringify(e);e=new H("MEGA"+e);var c=new H(16*Math.ceil(e.length/16));c.fill(0);e.copy(c);return c};O.unpackAttributes=function(e){for(var c=e.length;!e.readUInt8(c-1);)c--;e=e.slice(0,c).toString();if('MEGA{"'!==e.substr(0,6))throw Error("Attributes could not be decrypted with provided key.");return JSON.parse(e.substr(4).replace(/\0|[^}]*$/g,''))};var Ga=B(function(e){function c(a,b){if(a.forEach)return a.forEach(b);for(var d=0;d<a.length;d++)b(a[d],d)}var a=
...
function(e){if(typeof ReadableStream==='undefined')return e.waitUntil(Promise.reject());new ReadableStream;e.waitUntil(self.skipWaiting())});self.addEventListener("activate",function(e){e.waitUntil(self.clients.claim())});self.addEventListener("fetch",function(e){var c=(pb(e.request.url).search||"").substr(1),a="!"===c.charAt(0)?"https://mega.nz/#"+c:null;a&&(c=new Promise(function(b,c){var e=L.file(a);e.loadAttributes(function(a,f){if(a)return c(a);b(new Response(new ReadableStream({start:function(a){var b=e.download();b.on("data",function(b){return a.enqueue(new Uint8Array(b))});
b.on("end",function(){return a.close()})}}),{headers:{"Content-Length":f.size,"Content-Type":"application/octet-stream; charset=utf-8","Content-Disposition":"attachment; filename="+f.name}}))})}),e.respondWith(c["catch"](function(a){setTimeout(function(){fetch("https://api.rollbar.com/api/1/item/",{method:'POST',body:JSON.stringify({access_token:"e7dce51c0e174102bad8179a5c5680ac",data:{environment:"production",platform:"browser",client:{javascript:{browser:navigator.userAgent}},body:{message:{body:a.stack||a||'Unknown error (error is undefined)'}}}})})},100);return new Response(new Blob(["Unknown error!\n",
